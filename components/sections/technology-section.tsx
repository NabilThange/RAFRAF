...
function ScrollRevealText({ text }: { text: string }) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const startOffset = windowHeight * (isMobile ? 0.8 : 0.9);
      const endOffset = windowHeight * (isMobile ? 0.2 : 0.1);
  ...
  return (
    ...
    >
      {words.map((word, index) => {
        ...
        const wordBlur = (1 - wordAppearProgress) * maxBlur;
        ...
        return (
          <span
            ...
            style={{
              opacity: wordOpacity,
              filter: wordBlur > 2 ? `blur(${wordBlur}px)` : 'none', // Skip blur when almost 0
              transition: isMobile ? 'opacity 0.2s ease, filter 0.2s ease' : 'opacity 0.1s linear, filter 0.1s linear',
              marginRight: '0.3em',
              willChange: wordBlur > 2 ? 'opacity, filter' : 'opacity',
            }}
          >
            {word}
          </span>
...
export function TechnologySection() {
  ...
  const descriptionText = "Precision supply chain reimagining global produce trade. Certified cold storage, rigorous phytosanitary compliance and real-time tracking combine with direct farmer sourcing to create a seamless export pipeline. Handpicked varieties like fresh onions, tomatoes and leafy greens ensure premium quality and minimal spoilage at arrival.";
  ...
  return (
    <section ref={sectionRef} className="relative bg-foreground">
      ...
      <div className="sticky top-0 h-screen overflow-hidden">
        <div className="flex h-full w-full items-center justify-center">
          ...
          <div 
            ...
            style={{ gap: `${gap}px`, padding: `${imageProgress * 8}px` }}
          >
            
            {/* Left Column - Hidden on mobile */}
            <div 
              className="hidden sm:block relative overflow-hidden will-change-transform"
              style={{
                width: `${sideWidth}%`,
                height: "100%",
            ...
            >
              {sideImages.filter(img => img.position === "left").map((img, idx) => (
                <Image
                  ...
                  sizes="(max-width: 640px) 0vw, (max-width: 1024px) 22vw, 22vw"
                />
              ))}
            </div>
              ...
              >
                {textCycles.map((text, cycleIndex) => {
                  ...
                  return (
                    <h2 
                      key={cycleIndex}
                      className="absolute max-w-3xl font-medium leading-tight tracking-tight text-white text-[clamp(2rem,8vw,5rem)] sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl"
                    >
                      {words.map((word, wordIndex) => {
                        let wordOpacity = 0;
                        let wordBlur = maxBlur;
                        
                        if (scrollProgress >= cycleStart && scrollProgress < cycleEnd) {
                          const localProgress = (scrollProgress - cycleStart) / (cycleEnd - cycleStart);
                          
                          // First half: appear (blur maxBlur→0, opacity 0→1)
                          if (localProgress < 0.5) {
                            ...
                            wordBlur = (1 - wordAppearProgress) * maxBlur;
                          } 
                          // Second half: disappear (blur 0→maxBlur, opacity 1→0)
                          else {
                            ...
                            wordBlur = wordDisappearProgress * maxBlur;
                          }
                        }
                        ...
                        return (
                          <span
                            ...
                            style={{
                              opacity: wordOpacity,
                              filter: wordBlur > 2 ? `blur(${wordBlur}px)` : 'none',
                              transition: isMobile ? 'opacity 0.2s ease, filter 0.2s ease' : 'opacity 0.1s linear, filter 0.1s linear',
                              marginRight: '0.3em',
                              willChange: wordBlur > 2 ? 'opacity, filter' : 'opacity',
                            }}
      ...
      <div 
        ref={textSectionRef}
        className="relative overflow-hidden px-4 py-16 sm:px-6 sm:py-20 md:px-12 md:py-24 lg:px-20 lg:py-32 xl:py-40 bg-black"
      >
